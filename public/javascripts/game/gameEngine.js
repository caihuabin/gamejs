...
define(function(){
  var GameEngine = function (gameName, canvasId) {
  ...
  GameEngine.prototype = {
    ...
    getCanvas: function (canvasName) {
      return this.canvasObjs[canvasName];
    },
    queueCanvasFun: function (canvasFun) {
      this.canvasFuns.push(canvasFun);
    },
    loadCanvasFuns: function(){
      if (this.canvasIndex < this.canvasFuns.length) {
        try{
          this.loadCanvasFun(this.canvasFuns[this.canvasIndex]);
          this.canvasLoaded++;
        }
        catch(err){
          this.canvasFailedToLoad++;
          alert('error!');
        }
        
        this.canvasIndex++;
      }
      return (this.canvasLoaded + this.canvasFailedToLoad) / this.canvasFuns.length * 100;
    },
    loadCanvasFun: function (canvasFun) {
      this.canvasObjs[canvasFun.name] = canvasFun.fun();
    },
    getLoadingPercentComplete : function(){
      return (this.imagesLoaded + this.imagesFailedToLoad + this.canvasLoaded + this.canvasFailedToLoad) /
            (this.imageUrls.length + this.canvasFuns.length) * 100;
    },
    // Game loop..................................................................
    ...
    keyUp: function(e){
      var listener = undefined,
         key = undefined;
      switch (e.keyCode) {
        case 32: key = 'space up';       break;
        case 37: key = 'left arrow up';  break;
        case 39: key = 'right arrow up'; break;
        case 38: key = 'up arrow up';    break;
        case 40: key = 'down arrow up';  break;
      }
      listener = this.findKeyListener(key);
      if (listener) { // listener is a function
        listener();  // invoke the listener function
    ...
    addMouseListener: function (mouseAndListener) {
      this.mouseListeners.push(mouseAndListener);
    },
    findMouseListener: function (key) {
      var listener = undefined;
      for(var i=0; i < this.mouseListeners.length; ++i) {
        var mouseAndListener = this.mouseListeners[i],
            currentKey = mouseAndListener.key;
        if (currentKey === key) {
          listener = mouseAndListener.listener;
        }
      };
      return listener;
    },
